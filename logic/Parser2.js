...
class Parser {
			...
			switch (this._tokens[this._current].data.type) {
				case this._tokenTypes.SERVICE: {
					switch (this._tokens[this._current].data.value) {
						case this._triggers.ForStatement: {
							console.log("--Transfer control to _forStatementHandler--");
							BlockStatement.body.push(this._forStatementHandler(this._nextToken()));
							break;
						}
	...
	_expressionStatementHandler(prevToken) {
		...
		switch (token.data.value) {
			...
			case this._symbols.QUAD_OPEN: {
				// ArrayExpression
				console.log("ARRAY EXPRESSION");
				let index = this._nextToken();
				...
				return {
					type: this.statements.ArrayExpression,
					target: prevToken.data.value,
	...
	_forStatementHandler(token) {
		token = this._nextToken();
		console.log(token);
		// HERE handle of init
		let inBraces = [];
		while (token.data.value != this._symbols.BR_CLOSE) {
			inBraces.push(token.data.value);
			token = this._nextToken();
		}
		inBraces = inBraces.join(" ").split(" ; ");

		let testExpression = this._binaryExpressionHandler(inBraces[1].split(" "));
		let updateExpression = inBraces[2];
		// let updateExpression = this._binaryExpressionHandler(inBraces[2].split(' '));
		...
		this._nextToken();
		return {
			type: this.statements.ForStatement,
			init: inBraces[0], //TODO
			test: testExpression,
